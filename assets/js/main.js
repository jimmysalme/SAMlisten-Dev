jQuery(function($){
	...
	$('.sliderGallery,.trigger_slider').click(function(e){
        ...
        var $this=$(this), 
            ...
            p = $this.parents(".modal"), 
            gallayoutOption = p.find('.gallayoutOption'),
            gal_list_parent =p.find('.gal_list'),
			gal_list =p.find('.gal_list_child_'+indexVideo),
            socialShare = p.find('.social_share');
			
		gallayoutOption.children("li").removeClass('active');
		$this.parent('li').addClass('active');

		gal_list_parent.children("li").each(function() {
			$(this).removeClass('trigger_slider').addClass('gallery-item');

			$(this).hide();
			if ($(this).hasClass('trigger_slider_'+indexVideo)) {
			  $(this).show();
			}
        });
		
		$(".gal_list_mini").hide();
		$(".gal_list_iframevideo").show();
		...
		gal_list.addClass('owl-carousel owl-gallery');
		 ...
		 //gal_list.trigger('owl.goTo', indexVideo) 
		 socialShare.slideDown();
	});

	
	$('.gridGallery, .destroy_owl_video').on('click', function (e) {
        e.preventDefault();
        var $this=$(this), 
            p = $this.parents(".modal"),
            gallayoutOption = p.find('.gallayoutOption'),
		...
		if(owlGal.length){
			...
			owlGal.removeClass('owl-carousel owl-gallery');
		}
	});
    ...
    if($("#contactform").length!=0){
        ...
        $("#contactform").submit(function (e) {
            ...
            var s_spam_textbox1 = $("#form_val_1").val(),
            s_spam_textbox2 = $("#form_val_2").val(),
			name = $("#name").val(),
            email = $("#email").val(),
            subject = $("#subject").val(),
            ...
            if (message === '') {
                $('#valid-issue').html('Olvidaste escribir tu mensaje. ').slideDown();
				enviarMail = "false";
            }
			if (subject === '') {
                $('#valid-issue').html('Sobre qué asunto nos quieres hablar? ').slideDown();
				enviarMail = "false";
            }
			if (!IsEmail(email)) {
                $('#valid-issue').html('Recuerda escribir un email válido. ').slideDown();
				enviarMail = "false";
            }
			if (name === '') {
                $('#valid-issue').html('Por favor dinos tu nombre. ').slideDown();
				enviarMail = "false";
            }
			
			
			
			if(enviarMail == "true" && s_Bot1 == "false" && s_Bot2 == "false"){
				 var datos = {
						"name" : name,
						"email" : email,
						"subject" : subject,
				 ...
				 $.ajax({
						...
						beforeSend: function () {
							...
							//$("#c_send_sup").val("Envoi en cours...");
							$('#valid-issue').html('Estamos enviando tu mensaje, sólo aguarda un poco. ').slideDown();
						 },
						 success: function () {
							 $('#contactform').slideUp();
							 $('#valid-issue').html('Gracias por escribirnos. Tu mensaje ha sido enviado.<br> Nos pondremos en contacto contigo lo antes posible.').show();
						 }
				 });
			}
	...
	**************************/
	...
	if ($("#letra").prop('scrollHeight') < $("#letra").height() ) {
	  //your element don't have overflow
	  $("#panTop").hide();
	  $("#panBottom").hide();
	}
	...
	//Toogle Dedícala
	/*$("#btn_fb_ded").click(function() { 
		$('#cnt_fb_ded').slideToggle();
		
		if($('#cnt_mail_ded').is(':visible')) {
			$('#cnt_mail_ded').slideToggle();
		}
	});*/
	
	$( "#btn_mail_ded" ).click(function() {     
	   $('#cnt_mail_ded').slideToggle();
	   
	   if($('#cnt_fb_ded').is(':visible')) {
			$('#cnt_fb_ded').slideToggle();
		}
	});
